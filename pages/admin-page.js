...
export default function AdminDashboard() {
  ...
  useEffect(() => {
    setEnvPassword(process.env.NEXT_PUBLIC_ADMIN_PASSWORD || 'lexion123'); // 기본값
  }, []);
  ...
  const handleAddOrUpdateProduct = async () => {
    ...
    if (form.id) {
      ...
      setProducts(
        products.map(p => (p.id === form.id ? { id: form.id, ...newProduct } : p))
      );
    } else {
            ...
            <ul className="space-y-2">
              {products.map((product) => (
                <li key={product.id} className="border p-3 rounded">
                  ...
                  {product.imageUrl && (
                    <img src={product.imageUrl} alt="" className="w-32 mt-2" />
                  )}
